...
export async function generateMetadata({ params }) {
  ...
  return {
    title: categoryData.title + " | conference",
  }
}
                  ...
                  <div className="col-lg-7">
                      ...
                      <div className="mil-filter mil-up mil-mb-90">
                          <div className="mil-filter-links">
                              <Link href="/conference">All</Link>
                              {categories.map((item, key) => (
                              <Link key={`categories-item-${key}`} href={`/conference/category/${item.id}`} className={item.id == params.id ? "mil-current": ""}>{item.title}</Link>
                              ))}
                          </div>
                      </div>
                      {/* filter end */}
